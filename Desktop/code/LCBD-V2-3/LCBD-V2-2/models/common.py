...
from utils.activations import Mish  #导入激活函数
...
class DWConvblock(nn.Module):
    def __init__(self, c1, c2, size):
        ...
        self.block = nn.Sequential(nn.Conv2d(c1, c2, 1, 1, 0, bias=False),
                                   nn.BatchNorm2d(c2),
                                   nn.ReLU(inplace=True),

                                   nn.Conv2d(c2, c2, size, 1, 2, groups=c2,
                                             bias=False),
                                   nn.BatchNorm2d(c2),
                                   nn.ReLU(inplace=True),

                                   nn.Conv2d(c2, c2 // 2, 1, 1, 0, bias=False),
                                   nn.BatchNorm2d(c2 // 2),

                                   nn.Conv2d(c2 // 2, c2 // 2, size, 1, 2,
...
class Shuffle_Block(nn.Module):
    def __init__(self, inp, oup, stride):
        ...
        self.branch2 = nn.Sequential(
            ...
            nn.BatchNorm2d(branch_features),
            nn.ReLU(inplace=True),
            # CBAM(inp if (self.stride > 1) else branch_features),
            self.depthwise_conv(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=1),
            nn.BatchNorm2d(branch_features),
            nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False),
            nn.BatchNorm2d(branch_features),
