...
The root of this problem lies in the design of the sensor.<br>
Usually the signal generated by a particle is very weak, so in order to detect the particles, we should first consider amplifying the signal, but the gain should not be too large, otherwise the SNR will drop.Therefore, the detection target of the sensors is the signal with low output gain value (Gain ~10).<br>
Due to the requirement of fast resolution, it is necessary to consider how to quickly collect the induced charge.<br>
LGAD is one of the detectors designed based on the above considerations. In short, LGAD is a front-end detector that quickly senses incident particles and outputs signals.
###4b) Main Principle
####(1) Structure
